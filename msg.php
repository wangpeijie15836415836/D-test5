<?php
	$type = isset($_GET['type'])&&in_array($_GET['type'], array(1,2)) ? $_GET['type'] : 2;//设置type只在1和2之间
	$msg = isset($_GET['msg']) && !empty($_GET['msg']) ? $_GET['msg'] : "操作失败"; //isset()判断存在
	$url = isset($_GET['url']) && !empty($_GET['url']) ? $_GET['url'] : null;//存在的话给它获取的值，不存在为null
